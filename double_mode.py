...
def random_food():
    ...
    if food1 is not None:
        game_board.delete(food1)
    if food2 is not None:
        game_board.delete(food2)
...
def create_food():
    ...
    return game_board.create_oval(x, y, x + PIECE_SIZE, y + PIECE_SIZE, fill="purple")

def check_food_collision():
    head_coords = game_board.coords(snake_piece[0])

    if game_board.coords(snake_piece[0]) == game_board.coords(food1):
        update_score()
        return True, 1
    elif game_board.coords(snake_piece[0]) == game_board.coords(food2):
        update_score()
        return True, 2
    return False, 0
...
def end_game():
    ...
    if not check_collision():
        ...
        food_collision, food_eaten = check_food_collision()
...
def double_game():
    global food1, food2, snake_piece, direction, score, root, game_board, game_score
    ...
    root = tk.Tk()
    root.title("Double mode Snake Game")
